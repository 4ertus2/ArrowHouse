...
void StackTrace::forEachFrame(
    const StackTrace::FramePointers & frame_pointers, size_t offset, size_t size, std::function<void(const Frame &)> callback, bool fatal)
{
#if defined(__ELF__) && !defined(OS_FREEBSD)
    const AH::SymbolIndex & symbol_index = AH::SymbolIndex::instance();
    std::unordered_map<std::string, AH::Dwarf> dwarfs;

    using enum AH::Dwarf::LocationInfoMode;
    const auto mode = fatal ? FULL_WITH_INLINE : FAST;

    for (size_t i = offset; i < size; ++i)
    {
        StackTrace::Frame current_frame;
        std::vector<AH::Dwarf::SymbolizedFrame> inline_frames;
        ...
        if (object)
        {
            ...
            if (std::error_code ec; std::filesystem::exists(current_frame.object.value(), ec) && !ec)
            {
                ...
                AH::Dwarf::LocationInfo location;
                if (dwarf_it->second.findAddress(uintptr_t(current_frame.physical_addr), location, mode, inline_frames))
                {
        ...
        for (const auto & frame : inline_frames)
        {
            ...
            const AH::String file_for_inline_frame = frame.location.file.toString();
    ...
    for (size_t i = offset; i < size; ++i)
    {
        ...
        split.erase(std::remove_if(split.begin(), split.end(), [](const std::string & x) { return x.empty(); }), split.end());
        assert(split.size() == 6);
...
std::string demangleAndCollapseNames(std::optional<std::string_view> file, const char * const symbol_name)
{
    if (!symbol_name)
        return "?";
    ...
    std::string haystack = demangle(symbol_name);
...
static void
toStringEveryLineImpl([[maybe_unused]] bool fatal, const StackTraceRefTriple & stack_trace, std::function<void(std::string_view)> && callback)
{
    ...
    {
        AH::WriteBufferFromOwnString out;
        ...
        {
            out << " @ ";
            AH::writePointerHex(frame.physical_addr, out);
        }
...
{
    toStringEveryLineImpl(
        true,
        {frame_pointers, 0, static_cast<size_t>(std::ranges::find(frame_pointers, nullptr) - frame_pointers.begin())},
        std::move(callback));
}
