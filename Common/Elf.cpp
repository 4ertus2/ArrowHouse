...
void Elf::init(const char * data, size_t size, const std::string & path_)
{
    ...
    if (elf_size < sizeof(ElfEhdr))
        throw Exception(fmt::format("The size of supposedly ELF file '{}' is too small", path));

    header = reinterpret_cast<const ElfEhdr *>(mapped);

    if (memcmp(header->e_ident, "\x7F""ELF", 4) != 0)
        throw Exception(fmt::format("The file '{}' is not ELF according to magic", path));
    ...
    if (!section_header_offset
        ...
        throw Exception(fmt::format("The ELF '{}' is truncated (section header points after end of file)", path));
    ...
    if (!section_names_strtab)
        throw Exception(fmt::format("The ELF '{}' doesn't have string table with section names", path));

    ElfOff section_names_offset = section_names_strtab->header.sh_offset;
    if (section_names_offset >= elf_size)
        throw Exception(fmt::format("The ELF '{}' is truncated (section names string table points after end of file)", path));
    ...
    if (!program_header_offset
        ...
        throw Exception(fmt::format("The ELF '{}' is truncated (program header points after end of file)", path));
...
const char * Elf::Section::name() const
{
    if (!elf.section_names)
        throw Exception("Section names are not initialized");
