...
{
    AllocatorBufferDeleter(Allocator & allocator_, size_t size_) : allocator(allocator_), size(size_) { }
...
public:
    ...
    static constexpr size_t initial_buffer_bytes = Grower::initial_count * sizeof(Cell);
    static_assert(allocatorInitialBytes<Allocator> == 0 || allocatorInitialBytes<Allocator> == initial_buffer_bytes);
...
protected:
    ...
    void ALWAYS_INLINE
    emplaceNonZeroImpl(size_t place_value, KeyHolder && key_holder, LookupResult & it, bool & inserted, size_t hash_value)
    {
        it = &buf[place_value];
...
public:
    ...
    std::enable_if_t<Grower::performs_linear_probing_with_single_step, bool> ALWAYS_INLINE erase(const Key & x, size_t hash_value)
    {
        ...
        */
        ...
        while (true)
        {
            ...
            if (next_position > erased_key_position && (optimal_position > erased_key_position) && (optimal_position < next_position))
                continue;
