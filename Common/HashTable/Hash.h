...
template <typename T>
inline typename std::enable_if<(sizeof(T) > sizeof(UInt64)), UInt64>::type intHashCRC32(const T & x, UInt64 updated_value)
{
...
inline size_t DefaultHash64(std::enable_if_t<(sizeof(T) > sizeof(UInt64)), T> key)
{
    ...
    {
        return intHash64(
            static_cast<UInt64>(key) ^ static_cast<UInt64>(key >> 64) ^ static_cast<UInt64>(key >> 128) ^ static_cast<UInt64>(key >> 256));
    }
    assert(false);
    __builtin_unreachable();
}
...
struct UInt256Hash
{
    size_t operator()(UInt256 x) const
    {
        ...
        return CityHash_v1_0_2::Hash128to64(
            {CityHash_v1_0_2::Hash128to64({x.items[0], x.items[1]}), CityHash_v1_0_2::Hash128to64({x.items[2], x.items[3]})});
    }
};
