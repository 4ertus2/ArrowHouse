...
#define SAFE_CHECK(cond, ...) do { if (!(cond)) throw Exception(__VA_ARGS__); } while (false)
...
{
    SAFE_CHECK(len <= sp.size(), fmt::format("invalid string length: {} vs. {}", len, sp.size()));
    ...
    const CompilationUnit & cu, const Die & die, uint64_t attr_name) const
{
    ...
    switch (form)
    {
        ...
        case DW_FORM_ref_sig8:
            ...
            throw Exception(fmt::format("Type signatures are not supported (DIE at 0x{:x}, attr 0x{:x}).", die.offset, attr_name));

        case DW_FORM_ref_sup4:
        case DW_FORM_ref_sup8:
            throw Exception(fmt::format("Supplementary object files are not supported (DIE at 0x{:x}, attr 0x{:x}).", die.offset, attr_name));

        default:
            throw Exception(fmt::format("Unexpected form of attribute 0x{:x}: 0x{:x} (DIE at 0x{:x}).", attr_name, form, die.offset));
    }
}
...
void Dwarf::LineNumberVM::init()
{
    ...
    SAFE_CHECK(version_ >= 2 && version_ <= 5, fmt::format("invalid version in line number VM: {}", version_));
    if (version_ == 5)
    {
