...
private:
    ...
    static size_t roundUpToPageSize(size_t s, size_t page_size) { return (s + page_size - 1) / page_size * page_size; }
    ...
    size_t nextSize(size_t min_next_size) const
    {
        ...
        else
        {
            ...
            size_after_grow = ((min_next_size + linear_growth_threshold - 1) / linear_growth_threshold) * linear_growth_threshold;
        }
...
public:
    ...
    /** Begin or expand a contiguous range of memory.
      ...
      */
    char * allocContinue(size_t additional_bytes, char const *& range_start, size_t start_alignment = 0)
    {
        /*
        ...
        if (!range_start)
        {
            ...
            char * result = start_alignment ? alignedAlloc(additional_bytes, start_alignment) : alloc(additional_bytes);
        ...
        char * new_range = start_alignment ? alignedAlloc(new_bytes, start_alignment) : alloc(new_bytes);
