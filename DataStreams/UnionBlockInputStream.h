...
public:
        ...
        exception_callback(exception_callback_)
    {
        ...
        if (num_children > 1)
        {
            Header header = children.at(0)->getHeader();
...
protected:
    void finalize()
    {
        ...
        if (!all_read)
        {
            ...
            {
                //std::cerr << "popping\n";
                std::ignore = output_queue.pop(res);
    ...
    Block readImpl() override
    {
        ...
        //std::cerr << "popping\n";
        std::ignore = output_queue.pop(received_payload);
    ...
    void readSuffix() override
    {
        ...
        if (!all_read && !isCancelled())
            throw std::runtime_error("readSuffix called before all data is read");
