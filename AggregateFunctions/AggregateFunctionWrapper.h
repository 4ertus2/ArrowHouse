...
public:
        ...
        arrow::compute::ExecContext * /*ctx*/) const override
    {
        ...
        AggregateDescription description{.function = getHouseFunction(types), .arguments = arg_positions, .column_name = result_name};
            ...
            for (auto & assign : opts->assigns)
            {
                if (assign.function != AggFunctionId::AGG_UNSPECIFIED)
                {
                    ...
                    descriptions.emplace_back(
                        AggregateDescription{.function = func, .arguments = arg_positions, .column_name = assign.result_column});
                }
                else
                {
