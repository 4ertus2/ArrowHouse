...
struct AggregateFunctionSumData
{
        ...
        size_t end) /// NOLINT
    {
        ...
        while (ptr < end_ptr)
        {
            if (arrow::bit_util::GetBit(condition_map, start))
                Impl::add(sum, *ptr);
            ++ptr;
            ++start;
        }
...
#endif
    void NO_SANITIZE_UNDEFINED merge(const AggregateFunctionSumData & rhs) { Impl::add(sum, rhs.sum); }
...
public:
    ...
    DataTypePtr getReturnType() const override { return std::make_shared<TResult>(); }
    ...
    void addBatchSinglePlace(
        size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn ** columns, Arena *) const override
    {
        const auto & column = assert_cast<const ColumnType &>(*columns[0]);
...
public:
    ...
    WrappedSum(std::string name) : ArrowAggregateFunctionWrapper(std::move(name)) { }
