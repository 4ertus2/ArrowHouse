...
struct AvgFraction
{
    ...
    double divide() const { return static_cast<double>(numerator) / denominator; }
};
...
public:
    ...
    DataTypePtr getReturnType() const override { return std::make_shared<arrow::DoubleType>(); }
...
public:
    ...
    void
    addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn ** columns, Arena *) const final
    {
...
private:
    void increment(AggregateDataPtr __restrict place, Numerator inc) const { this->data(place).numerator += inc; }
};

class WrappedAvg final : public ArrowAggregateFunctionWrapper
{
public:
    WrappedAvg(std::string name) : ArrowAggregateFunctionWrapper(std::move(name)) { }
