...
namespace impl
{
    ...
    constexpr AH::UInt8 unhexDigit(char c)
    {
        return hex_char_to_digit_table[static_cast<AH::UInt8>(c)];
    }
    ...
    struct HexConversionUInt
    {
        ...
        static TUInt unhex(const char * data)
        {
            ...
            {
                res = static_cast<AH::UInt16>(unhexDigit(data[0])) * 0x1000 + static_cast<AH::UInt16>(unhexDigit(data[1])) * 0x100
                    + static_cast<AH::UInt16>(unhexDigit(data[2])) * 0x10 + static_cast<AH::UInt16>(unhexDigit(data[3]));
            }
            ...
            else
            {
                ...
                {
                    res <<= 64;
                    res += HexConversionUInt<AH::UInt64>::unhex(data);
                }
            }
            return res;
        }
    ...
    struct HexConversion<CityHashUInt128>
    {
        ...
        {
            HexConversion<AH::UInt64>::hex(uint_.high64, out, table);
            HexConversion<AH::UInt64>::hex(uint_.low64, out + 16, table);
        }

        static CityHashUInt128 unhex(const char * data)
        {
            CityHashUInt128 res;
            res.high64 = HexConversion<AH::UInt64>::unhex(data);
            res.low64 = HexConversion<AH::UInt64>::unhex(data + 16);
            return res;
        }
    };
}
...
inline void writeHexByteUppercase(AH::UInt8 byte, void * out)
{
    memcpy(out, &impl::hex_byte_to_char_uppercase_table[static_cast<size_t>(byte) * 2], 2);
}

inline void writeHexByteLowercase(AH::UInt8 byte, void * out)
{
...
constexpr AH::UInt8 unhex(char c)
{
    return impl::unhexDigit(c);
}

/// Converts two hexadecimal digits to UInt8.
constexpr AH::UInt8 unhex2(const char * data)
{
    return unhexUInt<AH::UInt8>(data);
}

/// Converts four hexadecimal digits to UInt16.
constexpr AH::UInt16 unhex4(const char * data)
{
    return unhexUInt<AH::UInt16>(data);
}

/// Produces a binary representation of a single byte.
inline void writeBinByte(AH::UInt8 byte, void * out)
{
