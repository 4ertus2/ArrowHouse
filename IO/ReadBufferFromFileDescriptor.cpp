...
size_t ReadBufferFromFileDescriptor::readImpl(char * to, size_t min_bytes, size_t max_bytes, size_t offset) const
{
    ...
    while (bytes_read < min_bytes)
    {
        ...
        if (-1 == res && errno != EINTR)
        {
            ...
            throw ErrnoException(fmt::format("Cannot read from file {}", getFileName()));
        }
...
void ReadBufferFromFileDescriptor::prefetch(Priority)
{
    ...
    if (0 != posix_fadvise(fd, file_offset_of_buffer_end, internal_buffer.size(), POSIX_FADV_WILLNEED))
        throw ErrnoException("Cannot posix_fadvise");
#endif
}
...
off_t ReadBufferFromFileDescriptor::seek(off_t offset, int whence)
{
    ...
    {
        throw Exception("ReadBufferFromFileDescriptor::seek expects SEEK_SET or SEEK_CUR as whence");
    }
    ...
    else
    {
        ...
        if (!use_pread)
        {
            ...
            if (-1 == res)
                throw ErrnoException(fmt::format("Cannot seek through file {} at offset {}", getFileName(), seek_pos));

            /// Also note that seeking past the file size is not allowed.
            if (res != seek_pos)
                throw Exception(fmt::format("The 'lseek' syscall returned value ({}) that is not expected ({})", res, seek_pos));
...
void ReadBufferFromFileDescriptor::rewind()
{
    if (!use_pread)
    {
        ...
        if (-1 == res)
            throw ErrnoException(fmt::format("Cannot seek through file {}", getFileName()));
    }
