...
inline size_t writeFloatTextFastPath(T x, char * buffer)
{
    ...
    if (result <= 0)
        throw Exception("Cannot print floating point number");
    return result;
}
 ...
 */
inline void writeJSONString(const char * begin, const char * end, WriteBuffer & buf, bool json_escape_forward_slashes)
{
    writeChar('"', buf);
    for (const char * it = begin; it != end; ++it)
    {
        switch (*it)
        {
            ...
            case '\\':
                ...
                break;
            case '/':
                if (json_escape_forward_slashes)
                    writeChar('\\', buf);
                writeChar('/', buf);
                break;
            case '"':
...
inline void writeBinary(StringRef x, WriteBuffer & buf)
{
    writeStringBinary(x, buf);
}
inline void writeBinary(std::string_view x, WriteBuffer & buf)
{
    writeStringBinary(x, buf);
}
#if 0
...
inline void writeBinary(const StackTrace::FramePointers & x, WriteBuffer & buf)
{
    writePODBinary(x, buf);
}
...
inline void writeText(is_floating_point auto x, WriteBuffer & buf)
{
...
{
    writeText(magic_enum::enum_name(x), buf);
}
#endif

inline void writeText(std::string_view x, WriteBuffer & buf)
{
    writeString(x.data(), x.size(), buf);
}
...
template <typename T>
requires is_arithmetic_v<T>
inline void writeQuoted(const T & x, WriteBuffer & buf)
{
    writeText(x, buf);
}
...
inline void writeQuoted(std::string_view x, WriteBuffer & buf)
{
    writeQuotedString(x, buf);
}

inline void writeQuoted(StringRef x, WriteBuffer & buf)
{
    writeQuotedString(x, buf);
}
...
template <typename T>
requires is_arithmetic_v<T>
inline void writeDoubleQuoted(const T & x, WriteBuffer & buf)
{
    writeText(x, buf);
}
...
inline void writeDoubleQuoted(std::string_view x, WriteBuffer & buf)
{
    writeDoubleQuotedString(x, buf);
}

inline void writeDoubleQuoted(StringRef x, WriteBuffer & buf)
{
    writeDoubleQuotedString(x, buf);
}
...
template <typename T>
requires is_arithmetic_v<T>
inline void writeCSV(const T & x, WriteBuffer & buf)
{
    writeText(x, buf);
}
...
struct fmt::formatter<AH::UUID>
{
    ...
    auto format(const AH::UUID & uuid, FormatContext & context) const
    {
