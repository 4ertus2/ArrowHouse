...
inline void readStringBinary(std::string & s, ReadBuffer & buf, size_t MAX_STRING_SIZE = DEFAULT_MAX_STRING_SIZE)
{
    ...
    if (size > MAX_STRING_SIZE)
        throw Exception("Too large string size.");
...
void readVectorBinary(std::vector<T> & v, ReadBuffer & buf, size_t MAX_VECTOR_SIZE = DEFAULT_MAX_STRING_SIZE)
{
    ...
    if (size > MAX_VECTOR_SIZE)
        throw Exception("Too large vector size.");
...
ReturnType readIntTextImpl(T & x, ReadBuffer & buf)
{
        ...
        switch (*buf.position())
        {
            ...
            case '-': {
                if constexpr (is_signed_v<T>)
                    ...
                    throw Exception("Unsigned type must not contain '-' symbol");
                else
                    return ReturnType(false);
                break;
            }
    ...
    else if constexpr (check_overflow == ReadIntTextCheckOverflow::CHECK_OVERFLOW)
    {
        x = res;
...
inline void readUUIDText(UUID & uuid, ReadBuffer & buf)
{
    ...
    {
        s[size] = 0;
        throw Exception(std::string("Cannot parse uuid ") + s);
    }
...
inline bool
tryReadDateTime64Text(DateTime64 & datetime64, UInt32 scale, ReadBuffer & buf, const DateLUTImpl & date_lut = DateLUT::instance())
{
...
inline void readDateTimeText(LocalDateTime & datetime, ReadBuffer & buf)
{
    ...
    {
        s[size] = 0;
        throw Exception(std::string("Cannot parse datetime ") + s);
    }
...
template <typename T>
inline std::enable_if_t<is_arithmetic_v<T>, void> readBinary(T & x, ReadBuffer & buf)
{
    readPODBinary(x, buf);
}
...
inline void readBinary(Int128 & x, ReadBuffer & buf)
{
    readPODBinary(x, buf);
}
...
#if 0
inline void readBinary(Decimal32 & x, ReadBuffer & buf)
{
    readPODBinary(x, buf);
}
inline void readBinary(Decimal64 & x, ReadBuffer & buf)
{
    readPODBinary(x, buf);
}
inline void readBinary(Decimal128 & x, ReadBuffer & buf)
{
    readPODBinary(x, buf);
}
...
template <typename T>
inline std::enable_if_t<std::is_integral_v<T>, bool> tryReadText(T & x, ReadBuffer & buf)
{
...
template <typename T>
inline std::enable_if_t<std::is_floating_point_v<T>, bool> tryReadText(T & x, ReadBuffer & buf)
{
...
[[noreturn]] inline void readText(UInt128 &, ReadBuffer &)
{
     ...
     */
    throw Exception("UInt128 cannot be read as a text");
}
...
template <typename T>
inline std::enable_if_t<is_arithmetic_v<T>, void> readQuoted(T & x, ReadBuffer & buf)
{
    readText(x, buf);
}
...
template <typename T>
inline std::enable_if_t<is_arithmetic_v<T>, void> readDoubleQuoted(T & x, ReadBuffer & buf)
{
    readText(x, buf);
}
...
#if 0
inline void readCSV(String & x, ReadBuffer & buf, const FormatSettings::CSV & settings)
{
    readCSVString(x, buf, settings);
}
    ...
    while (!buf.eof() && *buf.position() != ']')
    {
        if (!first)
        {
            if (*buf.position() == ',')
                ...
                throw Exception("Cannot read array from text");
        }
...
template <typename T>
inline std::enable_if_t<!std::is_integral_v<T>, void> readTextWithSizeSuffix(T & x, ReadBuffer & buf)
{
    readText(x, buf);
}
