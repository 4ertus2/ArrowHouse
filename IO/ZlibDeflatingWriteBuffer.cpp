...
#pragma GCC diagnostic pop
    ...
    if (rc != Z_OK)
        throw Exception(std::string("deflateInit2 failed: ") + zError(rc) + "; zlib version: " + ZLIB_VERSION);
}
...
ZlibDeflatingWriteBuffer::~ZlibDeflatingWriteBuffer()
{
    try
    {
        ...
        if (rc != Z_OK)
            throw Exception(std::string("deflateEnd failed: ") + zError(rc));
    }
    catch (...)
    {
...
void ZlibDeflatingWriteBuffer::nextImpl()
{
    ...
    do
    {
        ...
        if (rc != Z_OK)
            throw Exception(std::string("deflate failed: ") + zError(rc));
    }
...
void ZlibDeflatingWriteBuffer::finish()
{
    ...
    while (true)
    {
        ...
        if (rc != Z_OK)
            throw Exception(std::string("deflate finish failed: ") + zError(rc));
    }
}
