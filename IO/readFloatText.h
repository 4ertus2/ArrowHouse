...
ReturnType readFloatTextPreciseImpl(T & x, ReadBuffer & buf)
{
        ...
        if (unlikely(res.ec != std::errc()))
        {
            if constexpr (throw_exception)
                throw Exception(fmt::format(
                    "Cannot read floating point value here: {}", String(initial_position, buf.buffer().end() - initial_position)));
            else
    ...
    else
    {
        ...
        {
            if constexpr (throw_exception)
                throw Exception(fmt::format("Cannot read floating point value here: {}", String(tmp_buf, num_copied_chars)));
            else
                return ReturnType(false);
        }
...
ReturnType readFloatTextFastImpl(T & x, ReadBuffer & in, bool & has_fractional)
{
    ...
    {
        if constexpr (throw_exception)
            throw Exception("Cannot read floating point value");
        else
            return false;
    }
    ...
    if constexpr (allow_exponent)
    {
        if (checkChar('e', in) || checkChar('E', in))
        {
            ...
            {
                if constexpr (throw_exception)
                    throw Exception("Cannot read floating point value: nothing after exponent");
                else
                    return false;
            }
    ...
    if (num_characters_without_sign == 0)
    {
        ...
        {
            if constexpr (throw_exception)
                throw Exception("Cannot read floating point value: no digits read");
            else
                return false;
        }
        ...
        if (*in.position() == '+')
        {
            ...
            {
                if constexpr (throw_exception)
                    throw Exception("Cannot read floating point value: nothing after plus sign");
                else
                    return false;
            }
            else if (negative)
            {
                if constexpr (throw_exception)
                    throw Exception("Cannot read floating point value: plus after minus sign");
                else
                    return false;
            }
        }
...
template <typename T>
bool tryReadFloatTextNoExponent(T & x, ReadBuffer & in)
{
    bool has_fractional;
...
template <typename T>
bool tryReadFloatTextExt(T & x, ReadBuffer & in, bool & has_fractional)
{
    return readFloatTextFastImpl<T, bool>(x, in, has_fractional);
}
template <typename T>
bool tryReadFloatTextExtNoExponent(T & x, ReadBuffer & in, bool & has_fractional)
{
    return readFloatTextFastImpl<T, bool, false>(x, in, has_fractional);
}
