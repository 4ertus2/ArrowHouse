...
struct Memory : /*boost::noncopyable,*/ Allocator
{
    ...
    Memory(Memory && rhs) noexcept { swap(rhs); }
...
private:
    static size_t withPadding(size_t value)
    {
        ...
        if (common::addOverflow<size_t>(value, pad_right, res))
            throw Exception("value is too big to apply padding");
...
public:
    explicit BufferWithOutsideMemory(Memory<> & memory_) : Base(memory_.data(), memory_.size()), memory(memory_)
    {
        Base::set(memory.data(), memory.size(), 0);
        Base::padded = false;
    }

    size_t getActualSize() { return Base::count(); }
