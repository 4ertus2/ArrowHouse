...
static void parseComplexEscapeSequence(Vector & s, ReadBuffer & buf)
{
    ++buf.position();
    if (buf.eof())
        throw Exception("Cannot parse escape sequence");
    ...
    switch (*buf.position())
    {
        ...
        case 'u':
        {
            ...
            if (4 != buf.read(hex_code, 4))
                return error("Cannot parse escape sequence: less than four bytes after \\u");
            ...
            else
            {
                ...
                if (code_point >= 0xD800 && code_point <= 0xDBFF)
                {
                    if (!checkString("\\u", buf))
                        return error("Cannot parse escape sequence: missing second part of surrogate pair");
                    ...
                    if (second_code_point >= 0xDC00 && second_code_point <= 0xDFFF)
                    {
                        ...
                        return error("Incorrect surrogate pair of unicode escape sequences in JSON");
                }
                else
                {
...
ReturnType readJSONStringInto(Vector & s, ReadBuffer & buf)
{
    ...
    if (buf.eof() || *buf.position() != '"')
        return error("Cannot parse JSON string: expected opening quote");
    ++buf.position();
    ...
    return error("Cannot parse JSON string: expected closing quote");
}
...
ReturnType readDateTextFallback(LocalDate & date, ReadBuffer & buf)
{
    ...
    {
        if constexpr (throw_exception)
            throw Exception("Cannot parse date: value is too short");
        return ReturnType(false);
    };
...
{
    if (buf.eof())
        throw Exception("Unexpected EOF for key '" + name_of_field.toString() + "'");
    ...
    else if (isNumericASCII(*buf.position()) || *buf.position() == '-' || *buf.position() == '+' || *buf.position() == '.') /// skip number
    {
        ...
        if (!tryReadFloatText(v, buf))
            throw Exception("Expected a number field for key '" + name_of_field.toString() + "'");
    }
    ...
    else if (*buf.position() == '[')
    {
        ...
        while (true)
        {
            ...
            else if (!buf.eof() && *buf.position() == ']')
            {
                ...
                throw Exception("Unexpected symbol for key '" + name_of_field.toString() + "'");
        }
    }
    ...
    {
        throw Exception("Unexpected symbol '" + std::string(*buf.position(), 1) + "' for key '" + name_of_field.toString() + "'");
    }
}
