...
public:
    ...
    void ignoreAll() { tryIgnore(std::numeric_limits<size_t>::max()); }
    ...
    void readStrict(char * to, size_t n)
    {
        ...
        if (n != read_bytes)
            throw Exception(fmt::format("Cannot read all data. Bytes read: {}. Bytes expected: {}.", read_bytes, std::to_string(n)));
    }
...
private:
    ...
    [[noreturn]] static void throwReadAfterEOF() { throw Exception("Attempt to read after eof"); }
};
