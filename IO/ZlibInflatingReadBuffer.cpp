...
#pragma GCC diagnostic pop
    ...
    if (rc != Z_OK)
        throw Exception(std::string("inflateInit2 failed: ") + zError(rc) + "; zlib version: " + ZLIB_VERSION);
}
...
bool ZlibInflatingReadBuffer::nextImpl()
{
    ...
    if (rc == Z_STREAM_END)
    {
        ...
        else
        {
            ...
            if (rc != Z_OK)
                throw Exception(std::string("inflateReset failed: ") + zError(rc));
            return true;
        }
    }
    if (rc != Z_OK)
        throw Exception(std::string("inflate failed: ") + zError(rc));
