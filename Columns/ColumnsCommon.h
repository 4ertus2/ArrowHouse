...
{
    return assert_cast<MutableColumnBinary &>(column).Append(std::string_view{value.data, value.size}).ok();
}

inline bool insertFixedString(MutableColumn & column, const StringRef & value)
{
    return assert_cast<MutableColumnFixedString &>(column).Append(std::string_view{value.data, value.size}).ok();
}

inline bool insertDecimal(MutableColumn & column, const StringRef & value)
{
    return assert_cast<MutableColumnDecimal &>(column).Append(std::string_view{value.data, value.size}).ok();
}
