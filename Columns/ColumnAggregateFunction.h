...
public:
    ...
    DataTypeAggregateFunction(const AggregateFunctionPtr & function_, const DataTypes & argument_types_, const Array & parameters_)
        : arrow::ExtensionType(arrow::uint64()), function(function_), argument_types(argument_types_), parameters(parameters_)
    {
    }
    ...
    virtual arrow::Result<std::shared_ptr<arrow::DataType>>
    Deserialize(std::shared_ptr<arrow::DataType> /*storage_type*/, const std::string & /*serialized_data*/) const override
    {
...
public:
    ColumnAggregateFunction(const std::shared_ptr<DataTypeAggregateFunction> & data_type)
        : arrow::ExtensionArray(data_type, *arrow::MakeArrayOfNull(arrow::uint64(), 0)), func(data_type->getFunction())
    {
    }

    explicit ColumnAggregateFunction(const std::shared_ptr<arrow::ArrayData> & data)
        : arrow::ExtensionArray(data), func(std::static_pointer_cast<DataTypeAggregateFunction>(data->type)->getFunction())
    {
    }
...
{
public:
    MutableColumnAggregateFunction(
        const std::shared_ptr<DataTypeAggregateFunction> & data_type_, arrow::MemoryPool * pool = arrow::default_memory_pool())
        : arrow::ArrayBuilder(pool), data_type(data_type_), builder(std::make_shared<arrow::UInt64Builder>(pool))
    {
    }
